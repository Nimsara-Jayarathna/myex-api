...
All admin endpoints are internal and unversioned. They are under:

`/internal/admin`

### Admin Auth

- `POST /internal/internal/admin/auth/login`
- `POST /internal/internal/admin/auth/otp/verify`
- `POST /internal/internal/admin/auth/otp/resend`
- `GET /internal/internal/admin/auth/otp/status`
- `POST /internal/internal/admin/auth/otp/cancel`
- `GET /internal/internal/admin/auth/session`
- `POST /internal/internal/admin/auth/logout`

OTP flow:

- `POST /internal/internal/admin/auth/login` validates email/password and starts an OTP challenge.
- API sends a 6-digit email OTP, stores the challenge in an HttpOnly `adminOtpChallengeToken` cookie, and returns `otpRequired: true` with challenge status fields.
- The admin frontend does not need to send `challengeToken`; verify/resend/status/cancel read the challenge from the cookie.
- Optional `challengeToken` body/query fallback is still supported for non-browser clients.
- `POST /internal/internal/admin/auth/otp/verify` consumes OTP once, clears the OTP challenge cookie, and issues the `adminAccessToken` cookie.
- Wrong OTP attempts reduce `remainingAttempts`; after max attempts, challenge is locked.
...
- `GET /internal/admin/users` (admin-auth required)
- `GET /internal/admin/users/:id` (admin-auth required)
- `PATCH /internal/admin/users/:id` (admin-auth required)
- `GET /internal/admin/users/:id/activity` (admin-auth required)
- `POST /internal/admin/users/:id/reset-password` (admin-auth required)
- `POST /internal/admin/users/:id/force-logout` (admin-auth required)
...
`GET /internal/admin/users/:id` response `data`:

- `id`, `name`, `email`, `status`, `categoryLimit`, `defaultCurrency`, `createdAt`, `lastLoginAt`, `role`

`PATCH /internal/admin/users/:id` request body:

- `email` (optional, unique)
- `status` (optional: `ACTIVE | INACTIVE | SUSPENDED`)
- `categoryLimit` (optional integer `0..1000`)

`GET /internal/admin/users/:id/activity` response `data`:

- `activity`: array of `{ event, details, date }`

`POST /internal/admin/users/:id/reset-password` behavior:
...
`POST /internal/admin/users/:id/force-logout` behavior:

- Increments `tokenVersion` only (token invalidation strategy without session table).

### Admin Currencies

- `GET /internal/admin/currencies` (admin-auth required)
- `GET /internal/admin/currencies/:id` (admin-auth required)
- `POST /internal/admin/currencies` (admin-auth required)
- `PATCH /internal/admin/currencies/:id` (admin-auth required)
- `POST /internal/admin/currencies/:id/set-default` (admin-auth required)
- `POST /internal/admin/currencies/:id/toggle-status` (admin-auth required)

`GET /internal/admin/currencies` optional query params:
...
`POST /internal/admin/currencies` and `PATCH /internal/admin/currencies/:id` request body:
...
- `GET /internal/admin/categories` (admin-auth required)
- `POST /internal/admin/categories` (admin-auth required)
- `PATCH /internal/admin/categories/:id` (admin-auth required)
- `POST /internal/admin/categories/:id/set-default` (admin-auth required)
- `DELETE /internal/admin/categories/:id` (admin-auth required)
- `PATCH /internal/admin/categories/settings` (admin-auth required)

`GET /internal/admin/categories` response `data`:
...
- `GET /internal/admin/system` (admin-auth required)
- `GET /internal/admin/system/provider-usage` (admin-auth required)
- `POST /internal/admin/system/backup/run` (admin-auth required)
- `GET /internal/admin/system/backup/:id` (admin-auth required)
- `GET /internal/admin/system/backup/:id/download` (admin-auth required)
- `POST /internal/admin/system/backup/:id/cancel` (admin-auth required)
- `GET /internal/admin/system/delete-requests` (admin-auth required)
- `POST /internal/admin/system/delete-requests` (admin-auth required; helper endpoint)
- `POST /internal/admin/system/delete-requests/:id/decision` (admin-auth required)

`GET /internal/admin/system` response `data`:
...
`GET /internal/admin/system/provider-usage` optional query params:

- `date` (valid ISO date/time)

`POST /internal/admin/system/backup/run` behavior:
...
`POST /internal/admin/system/delete-requests/:id/decision` request body:
