...
export class AuthV1Controller {
  ...
  async getSession(@Req() req: Request) {
    const user = await this.authService.getUserSession((req.cookies as Record<string, string> | undefined)?.accessToken);
    return { user: this.authService.sanitizeUser(user) };
  }

  @Post('refresh')
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const { user, tokens } = await this.authService.refreshUserSession((req.cookies as Record<string, string> | undefined)?.refreshToken);
    setAuthCookies(res, tokens);
    return { user: this.authService.sanitizeUser(user) };
  }
