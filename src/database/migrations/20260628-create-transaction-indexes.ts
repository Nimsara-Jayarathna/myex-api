...
export default {
  ...
  async down(connection: Connection): Promise<void> {
    await connection.collection('transactions').dropIndex('user_1_date_-1').catch(() => undefined);
    await connection.collection('transactions').dropIndex('user_1_status_1').catch(() => undefined);
  },
};
