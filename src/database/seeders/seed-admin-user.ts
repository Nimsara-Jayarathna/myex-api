...
async function main(): Promise<void> {
  ...
  if (!seedEmail || !seedPassword) {
    logger.info({ message: 'Admin bootstrap skipped: set ADMIN_SEED_EMAIL and ADMIN_SEED_PASSWORD.' });
    return;
  }
  ...
  if (existing) {
    logger.info({ message: 'Admin bootstrap skipped: seed admin already exists.', adminEmailHash: hashEmail(seedEmail) });
    await mongoose.disconnect();
    return;
  }
  const passwordHash = await bcrypt.hash(seedPassword, Number(process.env.BCRYPT_ROUNDS ?? 10));
  await AdminUser.create({ email: seedEmail, passwordHash, roles: ['super_admin'], isActive: true });
